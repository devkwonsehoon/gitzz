 ...
 *
 * @param {user} Github_username
 * @return {yesterdayCommit}
 */
const getYesterDayCommitCount = async (user) => {
  const { $, lst } = await requestUserToGithub(user);
  const today = dayjs().subtract(1, 'day').format('YYYY-MM-DD');

  let count;
  let date;
  let yesterdayCommit = 0;

  lst.each((index, commit) => {
    date = formatYYYYMMDD($(commit).attr('data-date'));
    count = Number($(commit).attr('data-count'));
    if (date === today) yesterdayCommit = count;
  });
 ...
 * @return {monthTotalCommit}
 */
const getSpecificMonthCommitCounts = async (user, date) => {
  const { $, lst } = await requestUserToGithub(user);
  // const month = dayjs().format('YYYY-MM');
  ...
  lst.each((index, commit) => {
    target = formatYYYYMM($(commit).attr('data-date'));
    if (target === date) {
      countCommit = Number($(commit).attr('data-count'));
      if (countCommit > 0) count += 1;
    }
  });
 ...
 * @return {monthTotalCommit}
 */
const getSpecificMonthTotalCommitCount = async (user, month) => {
  const { $, lst } = await requestUserToGithub(user);
...
module.exports = {
  ...
  getSpecificMonthCommitCounts,
  getSpecificMonthTotalCommitCount,
